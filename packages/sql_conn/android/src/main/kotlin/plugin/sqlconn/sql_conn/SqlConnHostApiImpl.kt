...
class SqlConnHostApiImpl : SqlConnHostApi {
    ...
    // Pasar extras JDBC via connectionId: "myid;instance=X;ssl=off"
    val parts = cfg.connectionId.split(";")
    val extras = parts.drop(1).joinToString(";")
    val base = "jdbc:jtds:sqlserver://${cfg.host}:${cfg.port}/${cfg.database}"
    return if (extras.isNotEmpty()) "$base;$extras" else base
}
